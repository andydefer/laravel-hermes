...
// src/Collections/CompletionResultDataCollection.php
...
use AndyDefer\LaravelHermes\Datas\CompletionResultData;

/**
 * Collection of CompletionResultData objects.
 *
 ...
 *
 * @method CompletionResultData|null first()
 * @method CompletionResultData|null last()
 * @method CompletionResultData|null find(callable $callback)
 ...
 */
final class CompletionResultDataCollection extends AbstractTypedCollection
{
    /**
     * Initializes an empty collection of completion result data.
     */
    public function __construct()
    {
        parent::__construct(CompletionResultData::class);
    }
    ...
    {
        return $this->filter(
            fn (CompletionResultData $record): bool => $record->field === $field
        );
    }
    ...
    {
        return $this->filter(
            fn (CompletionResultData $record): bool => $record->similarity >= $minSimilarity
        );
    }
    ...
    {
        return array_map(
            fn (CompletionResultData $record): string => $record->token,
            $this->items
        );
    }
    ...
    {
        return array_map(
            fn (CompletionResultData $record): string => $record->originalText,
            $this->items
        );
    }
    ...
    {
        return array_map(
            fn (CompletionResultData $record): float => $record->similarity,
            $this->items
        );
    }
    ...
    {
        return array_map(
            fn (CompletionResultData $record): string => $record->documentId,
            $this->items
        );
    }
     ...
     *
     * @return CompletionResultData|null The best match, or null if collection is empty
     */
    public function getBestMatch(): ?CompletionResultData
    {
        if ($this->isEmpty()) {
            return null;
        }
